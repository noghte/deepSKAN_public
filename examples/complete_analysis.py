...
# for i in range(200):
#     print(i)
#     X, Y, K, S, C, I = gen.__getitem__(i)
    #print(X.shape)
X, Y, K, S, C, I = gen.__getitem__(0) #103 restrcition is in onlineDataGenerator.py
print(X.shape, X.dtype)
X = np.loadtxt("TCPP-THF-2.csv", delimiter=",")
X = X[np.newaxis, :, :, np.newaxis] # Reshaping the X to (1,ROWS,COLS,1) to adapt with the original paper's code
#X = np.array([np.arange(gen.batch_size), X, np.array([1])]).reshape(1,ROWS,COLS,1)
print(X.shape, X.dtype)

# Initialize Deep Learning and Global/Target Analyzers
# dl_model = DLAnalyzer(model_path='RES-33L-15M-103C-5S-6-48E-53A')
dl_model = DLAnalyzer(model_path='resnet_10')

gta_model = GTAnalyzer(X, np.logspace(2, 6, ROWS))
...
for i in range(5):
    x_ta += sads[i]*plot_c[:,i].reshape([ROWS, 1])
x_ta -= sads[i]*np.full([ROWS, 1], 1)
...
plt.subplot(332)
plt.title('data')
plt.contourf(X[0,:,:,0].T, levels=COLS)

# Residuals of global analysis
plt.subplot(331)
plt.title('GA residuals')
plt.contourf(gta_model.err(fit.x).reshape([ROWS,COLS]).T, levels=128)
plt.colorbar()

# Residuals of target analysis
plt.subplot(334)
plt.title('TA residuals')
plt.contourf((x_ta-X[0,:,:,0]).T, levels=COLS)
plt.colorbar()
