...
class onlineDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, batch_size, epoch_size, class_id=None, debug_mode=False):
        ...
        self.X = np.zeros([self.batch_size, ROWS, COLS, 1]) # Images
        self.Y = np.zeros([self.batch_size, 103]) # Kinetic id vectors

        if self.debug_mode:
            self.data_K = np.zeros([self.batch_size, 5, 5]) # transfer Matrices
            self.S = np.zeros([self.batch_size, 5, COLS]) # Spectra
            self.C = np.zeros([self.batch_size, ROWS, 5]) # Transients (concentration traces)
        ...
        for l in range(self.batch_size):
            ...
            for i in range(self.num_s):
                ...
                for _ in range(num_gauss-1):
                    ...
                    g *= amplitude
                    self.data_s[i] += g
                self.data_s[i] += np.random.normal(0, noise/COLS, self.x_points)
            ...
            for i in range(5):
                self.data_pure += self.data_s[i]*self.plot_c[:,i].reshape([ROWS, 1])
            self.data_pure -= self.data_s[i]*np.full([ROWS, 1], 1)
            self.data_noise_n = normalize(self.data_pure)

            self.data_y = np.zeros(103)
            self.data_y[class_id] = 1

            self.X[l] = self.data_noise_n.reshape([ROWS, COLS, 1])
